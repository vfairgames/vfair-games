...
import { BetFailureStage, LIMBO_GAME_ID } from '@vfair/game-contracts';
import {
...
@Injectable()
export class LimboBetService {
  constructor(
    @InjectPinoLogger(LimboBetService.name)
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        const round = await tx.gameRound.create({
          data: {
            ...
            outcome: {
              targetMultiplier: input.request.gameData.targetMultiplier,
  ...
  private async settleRound(
    ...
    const resolvedOdds = createLimboOdds(input.gameConfig.rtp).calculate({
      targetMultiplier: input.request.gameData.targetMultiplier,
    });
    const rolledMultiplier = rollLimbo(
      input.round.rotation.serverSeed.serverSeed,
      input.round.rotation.clientSeed,
      input.round.nonce,
      input.gameConfig.rtp,
    );
    const won = isLimboWon({
      rolledMultiplier,
      targetMultiplier: resolvedOdds.targetMultiplier,
    });
    const payoutMultiplier = won ? resolvedOdds.targetMultiplier : 0;
    const profit = won
      ? calculateProfitOnWin(
          input.request.betAmount,
          resolvedOdds.targetMultiplier,
  ...
  private validatePlaceBetRequest(
    ...
    const validation = odds.validate({
      targetMultiplier: request.gameData.targetMultiplier,
    ...
    if (
      resolvedOdds.targetMultiplier !== request.gameData.targetMultiplier ||
