...
const decrementAutoBetCount = (): boolean => {
  const count = useDiceGameStore.getState().form.autoBetCount;
  ...
  useDiceGameStore.getState().patchForm({ autoBetCount: nextCount });
  return nextCount > 0;
};
...
export const runAutoBetSession = async (shouldStop: {
  ...
  const initialBetAmount = useDiceGameStore.getState().form.betAmount;
  ...
  try {
    while (!shouldStop.current) {
      ...
      const form = useDiceGameStore.getState().form;
      const result = await diceGameService.placeBet(form);

      applyBetSuccess(result);

      const betResultTransitionMs =
        useDiceGameStore.getState().betResultTransitionMs;
      ...
      if (nextBetAmount !== form.betAmount) {
        useDiceGameStore.getState().patchForm({ betAmount: nextBetAmount });
      }
    ...
    useDiceGameStore.getState().patchForm({ betAmount: initialBetAmount });
  }
};
...
export const usePlaceAutoBet = () => {
  ...
  const mutation = useMutation({
    mutationKey: diceQueryKeys.mutations.placeAutoBet,
    mutationFn: async () => {
