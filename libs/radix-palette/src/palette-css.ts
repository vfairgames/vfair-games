...
import type { GeneratedColors } from './generate-radix-colors';
import { generateRadixColors } from './generate-radix-colors';
import type { PartnerPalette } from './palette-types';

type GetColorScaleCssParams = {
  isDarkMode: boolean;
  name: string;
...
type GetPartnerThemePreviewCssParams = {
  lightColors: GeneratedColors;
  darkColors: GeneratedColors;
};

export const getPartnerThemePreviewCss = ({
  lightColors,
  darkColors,
}: GetPartnerThemePreviewCssParams): string => {
  const lightAccentName = getColorName(lightColors.accentScale[8]);
  const darkAccentName = getColorName(darkColors.accentScale[8]);

  const lightAccentColorsCss = getColorScaleCss({
    isDarkMode: false,
    name: lightAccentName,
...
export const buildPartnerThemeCss = (palette: PartnerPalette): string =>
  getPartnerThemePreviewCss({
    lightColors: generateRadixColors({
      appearance: 'light',
      accent: palette.lightAccent,
      gray: palette.lightGray,
      background: palette.lightBg,
    }),
    darkColors: generateRadixColors({
      appearance: 'dark',
...
export const getColorName = (value: string): string => {
...
export const resolvePartnerAccentColors = (
  palette: PartnerPalette,
): { lightAccentColor: string; darkAccentColor: string } => {
  ...
  return {
    lightAccentColor: getColorName(lightColors.accentScale[8]),
    darkAccentColor: getColorName(darkColors.accentScale[8]),
  };
};
